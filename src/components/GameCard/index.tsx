...
export function GameCard({ gameInfo }: gameCardProps) {
    ...
    return (
        <article
            className='border border-green-100 rounded-xl grid grid-rows-2 sm:grid-rows-1 sm:grid-cols-3 sm:flex-row gap-4 sm:gap-12 overflow-hidden hover:cursor-pointer'
            onClick={() => router.push(`/game/${gameInfo.id}`)}
        >
            <div className='h-full relative'>
                <Image src={gameInfo.img} fill alt='Zelda GB' className='object-fill' />
            </div>

            <div className='px-8 sm:px-0 sm:pr-8 py-4 sm:py-8 flex flex-col gap-4 justify-between sm:col-span-2'>
